...
export class PaginatedListHandler<TModel extends BaseModel, TPayload extends PaginatedListFetchPayload = PaginatedListFetchPayload> extends ListHandler<TModel, TPayload> {
  ...
  protected listFromResponse(state:BaseState<TModel>, action:FetchAction<TPayload>): TModel[] {
    ...
    let stateList = state.list && list.length > 0 ? state.list.slice(0, count) : [];
